...
for row in results:
    ...
    i.append(long(hrate))
    drate = row[6]
    lo.append(long(drate))
    mincome = row[7]
    bo.append(long(mincome))
    mrate = row[8]
    fd.append(long(mrate))
    print age, btravel, dept, gender, jobrole, hrate, drate, mincome, mrate
...
c = np.array(bo)
d = np.array(fd)
X = np.dstack((a,b,c,d))[0]
print "four dimensional array X: "
print(X)
cnt= len(X)
print cnt
print "\n"
